...
contract BasicRenter is Ownable2Step {
    ...
    /* nftContractAddress => tokenId => Rent */
    ...
    */
    ...
    /**
    * @notice returns current user of given NFT
    * @param _nftContractAddress - address of ERC721 contract
    * @param _tokenId - NFT token ID
    ...
    */
    function userOf(address _nftContractAddress, uint256 _tokenId) external view returns(address user) {
        if(rents[_nftContractAddress][_tokenId].until >= block.timestamp) {
            user = rents[_nftContractAddress][_tokenId].to;
        }
    }
    ...
    *       to block.timestamp
    *     - rents[_order1.nftContractAddress][_order1.tokenId].to must be _order2.to
    */
    function matchOrders(
        ...
        Rent storage rent = rents[_order1.nftContractAddress][_order1.tokenId];
        uint256 until;
        
        if(rent.until >= block.timestamp) {
            require(rent.to == _order2.to, "Already rented");
            until = rent.until + _order2.duration; // extend the rent period
        } else {
            until = block.timestamp + _order2.duration;
        }

        require(until <= _order1.expiration && until <= _order2.expiration, "Cannot rent for that long");
        ...
        emit OrdersMatched(_order1.nftContractAddress, _order1.tokenId, _order1.from, _order2.to, _order1.erc20Token, _order1.price, _order2.duration, until, fee);
    }
